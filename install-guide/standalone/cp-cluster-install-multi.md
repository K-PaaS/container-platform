...
K-PaaS 컨테이너 플랫폼 Deployment를 통해 쿠버네티스 멀티 클러스터를 구성하고 각 리소스를 통해 K-PaaS 컨테이너 플랫폼 포털 환경을 배포하여 대시보드, 데이터베이스, 레파지토리 등의 환경을 제공한다.

K-PaaS 컨테이너 플랫폼 클러스터에 필요한 인스턴스 환경으로는 아래 구성을 참고한다.

> 멀티 클라우드 배포에서 Install 인스턴스를 제외한 나머지 인스턴스는 클러스터마다 필요에 따라 별도 구성한다.
...
- 각 클라우드의 1번 Control Plane 노드와 동일한 네트워크 서브넷에 신규 인터페이스를 2개 생성
- 외부 통신을 위해 신규 생성한 인터페이스 2개에 각각 Public IP 할당
- 각 클라우드의 1번 Control Plane 노드에 신규 생성한 인터페이스 2개 할당
...
멀티 클러스터에서는 Ingress Nginx Controller, Istio EastWest Gateway 서비스에 설정을 진행한다.
- 추후 설정할 환경변수 "INGRESS_NGINX_PRIVATE_IP"의 값으로 1번 Control Plane 노드에 할당한 첫번째 인터페이스 Private IP 설정
- 추후 설정할 환경변수 "ISTIO_EASTWEST_PRIVATE_IP"의 값으로 1번 Control Plane 노드에 할당한 두번째 인터페이스 Private IP 설정
- 추후 설정할 환경변수 "ISTIO_EASTWEST_PUBLIC_IP"의 값으로 1번 Control Plane 노드에 할당한 두번째 인터페이스 Public IP 설정
