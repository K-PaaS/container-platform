...
시스템 구성은 2개의 Kubernetes Cluster(Master, Worker) 환경으로 구성되어 있다.<br>
Container Platform Cluster Deployment를 통해 2개의 Kubernetes Cluster를 설치하고 Pod를 통해 Database, Private registry 등 미들웨어 환경을 제공하여 Container Image로 Kubernetes Cluster에 Container Platform 포털 환경을 배포한다. <br>
총 필요한 VM 환경으로는 **Install VM: 1개 (Master VM으로 대체 가능), 클러스터 당 Master VM: 각 1개 이상, 클러스터 당 Worker VM: 3개 이상**이 필요하고 본 문서는 Kubernetes Cluster 환경을 구성하기 위한 Master VM 과 Worker VM 설치 내용이다.
...
| TCP | 15443 | Istio EastWest Gateway |  
...
- 1번 클러스터 Container Platform Cluster 설치에 필요한 환경변수를 정의한다. HostName, IP 정보는 다음을 통해 확인할 수 있다.
```
...
- 2번 클러스터 Container Platform Cluster 설치에 필요한 환경변수를 정의한다. 1번 클러스터와 동일한 방식으로 설정한다.
```
...
$ kubectl --context=cluster1 get pods -n kube-system
...
$ kubectl --context=cluster2 get pods -n kube-system
NAME                                          READY   STATUS    RESTARTS      AGE
calico-node-d8sg6                             1/1     Running   0             9m22s
calico-node-kfvjx                             1/1     Running   0             10m
calico-node-khwdz                             1/1     Running   0             10m
